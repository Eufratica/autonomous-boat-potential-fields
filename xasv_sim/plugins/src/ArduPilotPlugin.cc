...
//  NOT MERGED IN MASTER YET
  ...
  /// \brief Model VelocityEast from GPS
  double velocity_east;
  
  /// \brief Model VelocityNorth from GPS
  double velocity_north;  

  /// \brief Model VelocityUp from GPS
  double velocity_up;    
...
void ArduPilotPlugin::ReceiveMotorCommand()
{
  ...
  if (this->dataPtr->arduPilotOnline)
  {
    ...
    // ArduPilot FCS.
    waitMs = 1000; //neste ponto que temos o gargalo para comunicação com a BBB
  }
  else
  {
    // Otherwise skip quickly and do not set control force.
    waitMs = 1;  //neste ponto que temos o gargalo na inicialização (RTF=0.8)
  }
  ssize_t recvSize =
    this->dataPtr->socket_in.Recv(&pkt, sizeof(ServoPacket), waitMs);
    
    //gzdbg << "recvSize [" << 0 << "]: " << recvSize << "\n";
  ...
  else
  {
    ...
    if(pkt.motorSpeed[15]>-1.0) //TTR - ADDED TO SOLVE UNWANTED MOVEMENTS AT START
    {
     goControl = false;
    }
    else{
  ...
  if (!this->dataPtr->gpsSensor)
    {
        ...
        pkt.velocity_east = this->dataPtr->gpsSensor->VelocityEast(); 
        pkt.velocity_north = this->dataPtr->gpsSensor->VelocityNorth();         
        pkt.velocity_up = this->dataPtr->gpsSensor->VelocityUp();                                             
        //gzdbg << "altitude [" << pkt.longitude << "]: " << pkt.longitude << "\n";        
        //gzdbg << "latitude [" << pkt.latitude << "]: " << pkt.latitude << "\n";        
        //gzdbg << "altitude [" << pkt.altitude << "]: " << pkt.altitude << "\n";
        //gzdbg << "velocity_east [" << pkt.longitude << "]: " << pkt.velocity_east << "\n";        
        //gzdbg << "velocity_north [" << pkt.latitude << "]: " << pkt.velocity_north << "\n";        
        //gzdbg << "velocity_up [" << pkt.altitude << "]: " << pkt.velocity_up << "\n";        
    }
