...
void FollowPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
  ...
  else if (_sdf->HasElement("line"))
  {
    ...
    direction_angle = lineElem->Get<double>("direction");
    ...
    // <force> element is required
    if (!lineElem->HasElement("force"))
    {
      gzerr << "FollowPlugin: Unable to find <line><force> element "
            << "in SDF." << std::endl;
      return;
    }
    auto force = lineElem->Get<double>("force");
    this->forceToApply = force;
    
    // <torque> element is required
    if (!lineElem->HasElement("torque"))
    {
      gzerr << "FollowPlugin: Unable to find <line><torque> element "
            << "in SDF." << std::endl;
      return;
    }
    auto torque = lineElem->Get<double>("torque");
    this->torqueToApply = torque;
  ...
  // Parse the optional <frame> element.
  if (_sdf->HasElement("frame"))
  {
    std::string frame = _sdf->GetElement("frame")->Get<std::string>();
    if (frame == "world")
    {
      this->applyForceInWorldFrame = true;
    }
    else if (frame == "robot")
    {
      this->applyForceInWorldFrame = false;
    }
    else
    {
      gzerr << "FollowPlugin: Unknown <frame> value: " << frame << std::endl;
      return;
    }
  }
...
void FollowPlugin::Update()
{
  ...
  // Move commands. Apply force depending on the selected frame.
  if (this->applyForceInWorldFrame)
  {
    this->link->AddForce(ignition::math::Vector3d(this->forceToApply * cos(direction_angle), this->forceToApply * sin(direction_angle), 0));
  }
  else
  {
    this->link->AddLinkForce(ignition::math::Vector3d(this->forceToApply * cos(direction_angle), this->forceToApply * sin(direction_angle), 0));
  }

  // Apply torque for turning
  this->link->AddTorque({0, 0, -this->torqueToApply});
}
